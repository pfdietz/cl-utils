...
(deftest unroll-test ()
  (is (equal (eval '(let ((x 0))
                     (list
                      (unroll (dotimes (i 10) (incf x i)))
                      x)))
             '(nil 45)))
  (is (equal (eval '(let ((x 0))
                     (unroll (dotimes (i -1 x) (incf x)))))
             0))
  (is (equal (eval '(let ((x nil))
                     (list
                      (unroll (dolist (e '(a b c))
                                (declare (type symbol e))
                                (push e x)))
                      x)))
             '(nil (c b a))))
  (is (equal (eval '(unroll (dotimes (i 10 nil)
                              (declare (type integer i))
                              (when (> i 3) (return i)))))
             4))
  (is (equal (eval '(unroll (dolist (i '(0 1 7 3 5 10 2) nil)
                              (when (> i 3) (return i)))))
             7))
  (is (equal (eval '(let ((x '(a b)) (y nil))
                     (unroll (dolist (e x y) (push e y)))))
             '(b a)))
  (is (equal (eval '(let ((x 5) (y nil))
                     (unroll (dotimes(i x y) (push i y)))))
             '(4 3 2 1 0)))
  (is (equal (eval '(let ((y nil))
                     (unroll (dotimes (i 10 y)
                               (when (<= i 2) (go around))
                               (push i y)
                               around))))
             '(9 8 7 6 5 4 3)))
  (is (equal (eval '(let ((y nil))
                     (unroll (dolist (i '(0 1 2 3 4 5 6) y)
                               (when (<= i 2) (go around))
                               (push i y)
                               around))))
             '(6 5 4 3))))
